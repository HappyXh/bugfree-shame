var img = {
    ...
    init: function () {
        ...
        for (var i = 0; i < img.imagebg_li.length; i++) {
            ...
            if (i != 0) {
                img.small_pic_html.push("<li onclick='img.play(" + i + ")'><img src='" + img.imagebg_li[i].getAttribute("data-sPic") + "'  style='width:100%; height:100%;' /></li>");
                img.imagebg.getElementsByTagName("li")[i].style.display = "none";
            } else {
                img.small_pic_html.push("<li onclick='img.play(" + i + ")' class='currently'><img src='" + img.imagebg_li[i].getAttribute("data-sPic") + "' style='width:100%; height:100%;' /></li>");
            }
            //console.log(img.small_pic_html);
        }
        //console.log(img.small_pic_html)
        img.small_pic.innerHTML="";
    ...
    play: function (i) {
        if(i>=0 && i<img.count) {
            ...
            if (i > img.num) {
                ...
                if (parseInt(img.current.style.left) > 500) {
                    //如果活动框到了最右边--完成
                    //小图片向左
